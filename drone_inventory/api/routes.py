...
from drone_inventory.models import User, Drone, drone_schema, drone_schemas, db
...
@token_required
def create_drone(current_user_token): #coming from token_required decorator
...
def get_drones(current_user_token):
    ...
    response = drone_schemas.dump(drones)
    return jsonify(response)
